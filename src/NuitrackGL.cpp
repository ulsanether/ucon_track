...
#include < android / log.h > #define LOGGING(...) {
    __android_log_print(ANDROID_LOG_INFO, "NUITRACK", __VA_ARGS__);
}
#endif

using namespace tdv::nuitrack;

NuitrackGLSample::NuitrackGLSample(): _textureID(0),
_textureBuffer(0),
_width(640),
_height(480),
...
void NuitrackGLSample::init(const std::string & config) {
    ...
    _depthSensor -> connectOnNewFrame(
        std::bind(& NuitrackGLSample::onNewDepthFrame, this, std::placeholders::_1)
    );

    _colorSensor = ColorSensor::create();
    // Bind to event new frame
    _colorSensor -> connectOnNewFrame(
        std::bind(& NuitrackGLSample::onNewRGBFrame, this, std::placeholders::_1)
    );
    ...
    _userTracker -> connectOnUpdate(
        std::bind(& NuitrackGLSample::onUserUpdateCallback, this, std::placeholders::_1)
    );
    _userTracker -> connectOnNewUser(
        std::bind(& NuitrackGLSample::onNewUserCallback, this, std::placeholders::_1)
    );
    _userTracker -> connectOnLostUser(
        std::bind(& NuitrackGLSample::onLostUserCallback, this, std::placeholders::_1)
    );

    _skeletonTracker = SkeletonTracker::create();
    // Bind to event update skeleton tracker
    _skeletonTracker -> connectOnUpdate(
        std::bind(& NuitrackGLSample::onSkeletonUpdate, this, std::placeholders::_1)
    );

    _handTracker = HandTracker::create();
    // Bind to event update Hand tracker
    _handTracker -> connectOnUpdate(
        std::bind(& NuitrackGLSample::onHandUpdate, this, std::placeholders::_1)
    );

    _gestureRecognizer = GestureRecognizer::create();
    _gestureRecognizer -> connectOnNewGestures(
        std::bind(& NuitrackGLSample::onNewGesture, this, std::placeholders::_1)
    );

    _onIssuesUpdateHandler = Nuitrack::connectOnIssuesUpdate(
        std::bind(& NuitrackGLSample::onIssuesUpdate, this, std::placeholders::_1)
    );
}
...
bool NuitrackGLSample::update() {
    if (!_isInitialized) {
        ...
        // When Nuitrack modules are created, we need to call Nuitrack::run() to start
...
void NuitrackGLSample::onUserUpdateCallback(UserFrame::Ptr frame) {
    ...
    for (uint16_t label = 0; label < MAX_LABELS; ++label) {
        ...
        if (_issuesData) {
            FrameBorderIssue::Ptr frameBorderIssue = _issuesData -> getUserIssue<FrameBorderIssue>(
                label
            );
...
void NuitrackGLSample::onNewGesture(GestureData::Ptr gestureData) {
	...
	for (int i = 0; i < _userGestures.size(); ++i) {
		...
		if (_userGestures[i].type == 0) {
			...
			cout << "0000" << endl; //
		} else if (_userGestures[i].type != 0) {
			gesture_count_0 = 0;
		}
		}
